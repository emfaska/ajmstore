...
@section('page_title', 'Ubah Transaksi Pembelian')
...
<div class="max-w-6xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Ubah Transaksi Pembelian</h2>
            <p class="text-sm text-gray-500 font-normal">Edit informasi faktur pembelian barang masuk.</p>
        </div>
    </div>
    ...
    <form action="{{ route('purchases.update', $purchase->id) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
        @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="invoice_number" value="{{ old('invoice_number', $purchase->invoice_number) }}" required class="w-full rounded-lg border-gray-300 bg-gray-50 focus:border-blue-500 focus:ring-blue-500 text-sm font-semibold text-gray-700" placeholder="Contoh: INV-PRC-xxxx">
                    ...
                    <div>
                        ...
                        <input type="date" name="purchase_date" value="{{ old('purchase_date', $purchase->purchase_date->format('Y-m-d')) }}" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
                    ...
                    <div>
                        ...
                        @php
                            $activePaymentMethodId = old('payment_method_id', $purchase->cashTransactions->first()?->payment_method_id);
                        @endphp
                        <select name="payment_method_id" id="payment_method_id" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
                            ...
                            @foreach($paymentMethods as $method)
                                <option value="{{ $method->id }}" {{ $activePaymentMethodId == $method->id ? 'selected' : '' }}>
                                    {{ $method->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500" id="table-items">
                        ...
                        <tbody id="item-rows">
                            <!-- Existing item rows populated below -->
                        </tbody>
                    </table>
                </div>
            </div>
                ...
                <div class="space-y-4 text-sm">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
                            <option value="pending" {{ old('status', $purchase->status) === 'pending' ? 'selected' : '' }}>Draft (Pending)</option>
                            <option value="completed" {{ old('status', $purchase->status) === 'completed' ? 'selected' : '' }}>Selesai (Completed)</option>
                            <option value="cancelled" {{ old('status', $purchase->status) === 'cancelled' ? 'selected' : '' }}>Dibatalkan (Cancelled)</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="payment_status" id="payment_status" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm">
                            <option value="unpaid" {{ old('payment_status', $purchase->payment_status) === 'unpaid' ? 'selected' : '' }}>Belum Lunas (Unpaid)</option>
                            <option value="partially_paid" {{ old('payment_status', $purchase->payment_status) === 'partially_paid' ? 'selected' : '' }}>Sebagian (Partially Paid)</option>
                            <option value="paid" {{ old('payment_status', $purchase->payment_status) === 'paid' ? 'selected' : '' }}>Lunas (Paid)</option>
                        </select>
                    ...
                    <div class="flex justify-between font-medium text-gray-600">
                        ...
                        <span id="txt-subtotal-summary">Rp {{ number_format($purchase->total_amount, 2, ',', '.') }}</span>
                    </div>

                    <div class="flex justify-between font-bold text-gray-800 text-base border-t border-gray-100 pt-3">
                        <span>Grand Total:</span>
                        <span id="txt-grand-total" class="text-blue-600">Rp {{ number_format($purchase->total_amount, 2, ',', '.') }}</span>
                    </div>
                </div>

                <div class="border-t border-gray-100 pt-4 flex flex-col gap-3">
                    <button type="submit" class="w-full py-2.5 bg-yellow-600 text-white font-semibold rounded-lg shadow hover:bg-yellow-700 transition-colors text-center text-sm">
                        Perbarui Transaksi
                    </button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Tambah baris baru
    function addRow(selectedProductId = '', qty = 1, costPrice = 0) {
        ...
        products.forEach(p => {
            ...
            options += `<option value="${p.id}" data-price="${p.purchase_price}" ${isSelected}>${p.name} (Stok: ${p.stock})</option>`;
        });
        ...
        tr.innerHTML = `
            ...
            <td class="px-3 py-3">
                <input type="number" name="items[${rowIndex}][quantity]" required min="1" value="${qty}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm text-center input-qty">
            </td>
            <td class="px-3 py-3">
                <input type="number" step="0.01" name="items[${rowIndex}][cost_price]" required min="0" value="${costPrice}" class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm text-right input-price">
            </td>
            <td class="px-3 py-3 text-right font-semibold text-gray-800 txt-subtotal">
                ${formatRupiah(qty * costPrice)}
            </td>
    ...
    if (existingItems && existingItems.length > 0) {
        existingItems.forEach(item => {
            addRow(item.product_id, item.quantity, item.cost_price);
        });
    } else {
        addRow();
    }
