...
<div x-show="sidebarOpen" x-transition.opacity class="fixed inset-0 z-40 bg-gray-900 bg-opacity-50 lg:hidden" @click="sidebarOpen = false" x-cloak></div>

<!-- Sidebar Container -->
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 flex flex-col">
    ...
    <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto">
        ...
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 {{ request()->routeIs('dashboard') ? 'bg-blue-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }} rounded-lg transition-colors text-sm font-medium">
        ...
        <div x-data="{ open: {{ request()->routeIs('sales.*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-collapse class="pl-11 pr-3 py-2 space-y-1" x-cloak>
                ...
                <a href="{{ route('sales.index') }}" class="block py-1.5 text-sm {{ request()->routeIs('sales.*') && !request()->routeIs('sales.create') ? 'text-white font-semibold' : 'text-slate-400 hover:text-white' }}">Daftar Penjualan</a>
            </div>
        </div>
        ...
        <div x-data="{ open: {{ request()->routeIs('reports.*') ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-collapse class="pl-11 pr-3 py-2 space-y-1" x-cloak>
                <a href="{{ route('reports.cash') }}" class="block py-1.5 text-sm {{ request()->routeIs('reports.cash') ? 'text-white font-semibold' : 'text-slate-400 hover:text-white' }}">Laporan Kas</a>
                <a href="{{ route('reports.sales') }}" class="block py-1.5 text-sm {{ request()->routeIs('reports.sales') ? 'text-white font-semibold' : 'text-slate-400 hover:text-white' }}">Laporan Penjualan</a>
                <a href="{{ route('reports.purchases') }}" class="block py-1.5 text-sm {{ request()->routeIs('reports.purchases') ? 'text-white font-semibold' : 'text-slate-400 hover:text-white' }}">Laporan Pembelian</a>
            </div>
        </div>
        @endif
        ...
        <!-- Pengaturan -->
        @if(auth()->user()->isOwner() || auth()->user()->isAdmin())
        <a href="{{ route('settings.index') }}" class="flex items-center gap-3 px-3 py-2.5 {{ request()->routeIs('settings.*') ? 'bg-blue-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }} rounded-lg transition-colors text-sm font-medium mt-4">
